  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-3 rounded-md border border-paper-line-strong bg-paper/60 p-4 sm:grid-cols-[1fr_1fr_auto]">
        <div>
          <label className="block text-sm text-ink-faint">Período — de</label>
          <input
            type="date"
            value={periodStart}
            onChange={(e) => setPeriodStart(e.target.value)}
            className="mt-1 w-full rounded-md border border-paper-line-strong bg-paper px-3 py-2 text-base md:min-h-11"
          />
        </div>
        <div>
          <label className="block text-sm text-ink-faint">até</label>
          <input
            type="date"
            value={periodEnd}
        ...
        <button
          ...
          className="self-end rounded-md bg-chart px-4 py-2 text-base font-medium text-paper disabled:opacity-50 md:min-h-11"
        >
          {isPending ? "Gerando…" : "Gerar rascunho com IA"}
        </button>
      </div>

      {error && <p className="text-base text-status-negative-text">{error}</p>}
      {notice && <p className="text-base text-status-positive-text">{notice}</p>}

      {report && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div>
            <h2 className="text-base font-medium uppercase tracking-wide text-ink-soft">Rascunho da IA</h2>
            <p className="mt-1 text-sm text-ink-faint">
              {report.periodStart} a {report.periodEnd} — {STATUS_LABEL[report.status] ?? report.status}
            </p>
            <div className="mt-3 whitespace-pre-wrap rounded-md border border-paper-line-strong bg-paper/60 p-4 text-base text-ink-soft">
              {report.aiDraft || "—"}
            </div>
          </div>
          <div>
            <h2 className="text-base font-medium uppercase tracking-wide text-ink-soft">Versão final (editável)</h2>
            <textarea
              value={finalText}
              onChange={(e) => setFinalText(e.target.value)}
              disabled={!canEdit || isPending}
              rows={14}
              className="mt-3 w-full rounded-md border border-paper-line-strong bg-paper px-3 py-2 text-base disabled:opacity-70"
            />
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                disabled={!canEdit || isPending}
                onClick={handleSave}
                className="rounded-md border border-paper-line-strong px-4 py-2 text-base text-ink disabled:opacity-50 md:min-h-11"
              >
                Salvar edição
              </button>
              <button
              ...
              <button
                ...
                onClick={handleSend}
                className="rounded-md bg-gold px-4 py-2 text-base font-medium text-paper disabled:opacity-50 md:min-h-11"
              >

    ...
    <PageContainer className="bg-paper">
        ...
        {activeTab === "salas" && (
          <div className="space-y-6">
            ...
            {/* Cobertura de Estagiários por Especialidade — leitura de apoio,
                em formato compacto e no fim da aba: a capacidade das salas e da
                clínica é o indicador principal aqui. O alerta acima já responde
                "preciso de estagiário?"; este bloco só detalha o percentual
                (estagiários contratados / crianças com check-in na semana em
                atendimentos 1:1). 100% = proporção recomendada atingida. */}
              ...
              <div className="rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-xs">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <h3 className="text-sm font-bold text-slate-700">
                    Cobertura de Estagiários por Especialidade
                    <span className="ml-2 text-xs font-normal text-slate-400">semana atual · 1 estagiário por criança = 100%</span>
                  </h3>
                    ...
                    return (
                      <div key={item.specialtyId}>
                        <div className="flex items-baseline justify-between gap-2">
                          ...
                          <p className={`shrink-0 text-xs font-bold tabular-nums ${textColor}`}>
                            {item.status === "sem_demanda"
                              ? "—"
                              : `${item.coveragePct}% · ${item.internsCount}/${item.childrenCount}`}
                          </p>
                        </div>

...
/**
 * Corpo de gestão de guias (lista + vincular + cadastrar) extraído de
 * `GuiaQuickActionModal` pra ser reaproveitado também dentro do modal de
 * chegada de avaliação/acolhimento/anamnese (chegada-avaliacao-modal.tsx),
 * que embute isto junto com o upload de laudo, sem duplicar a lógica de
 * guia — só a moldura (header/footer) de cada modal é diferente.
 */
export function GuiaSection({
  ...
  /** Chamado depois de vincular (existente ou recém-cadastrada) com sucesso. */
  onLinked?: () => void;
}) {
  ...
  useEffect(() => {
    ...
    getPatientActiveAuthorizations(patientId).then((guides) => {
      ...
      setShowAddForm(guides.length === 0);
    });
  ...
  return (
    <div className="space-y-4">
      {error && (
        <div className="flex items-center gap-2 rounded-lg bg-rose-50 p-3 text-xs font-medium text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
          <AlertCircle className="h-4 w-4 shrink-0 text-rose-500" />
          <span>{error}</span>
        </div>
      )}
      {successMsg && (
        <div className="flex items-center gap-2 rounded-lg bg-emerald-50 p-3 text-xs font-medium text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="w-full max-w-xl max-h-[90vh] flex flex-col rounded-xl border border-paper-line bg-paper shadow-2xl transition-all">
        ...
        <div className="p-5 overflow-y-auto flex-1">
          <GuiaSection patientId={patientId} appointmentId={appointmentId} onLinked={() => setTimeout(() => { onSuccess?.(); onClose(); }, 1000)} />
        </div>

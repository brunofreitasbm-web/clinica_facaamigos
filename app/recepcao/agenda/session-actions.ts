 ...
 * (dados fixos, sem nenhuma consulta ao banco). Delega o cálculo puro para
 * lib/available-slots.ts (computeAvailableSlots), reaproveitado também pelo
 * bot de acolhimento de plano de saúde (lib/twilio-intake-bot.ts) com
 * duração/limite diferentes — mesma lógica, sem duplicação.
 */
export async function getAvailableSlots(
  ...
  const supabase = await createClient();
  return computeAvailableSlots(supabase, { roomId, therapistId, durationMinutes, excludeAppointmentId, limit: 8 });
}

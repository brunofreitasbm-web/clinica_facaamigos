...
function Pill({ pill }: { pill: LeadPill }) {
  ...
  return (
    <span
      ...
      className={`whitespace-nowrap rounded-full border px-2 py-0.5 text-[11px] font-medium ${PILL_STYLE[pill.state]}`}
    >
...
function SummaryChip({ pendencies }: { pendencies: LeadPendencies }) {
...
/** Linha recolhida da fila: chip "Faltam N" + uma pílula por item (sem os "não se aplica"). */
export function PendencyPills({ pendencies }: { pendencies: LeadPendencies }) {
  return (
    <span className="mt-1.5 flex flex-wrap items-center gap-1.5">
      <SummaryChip pendencies={pendencies} />
      {visiblePills(pendencies).map((pill) => (

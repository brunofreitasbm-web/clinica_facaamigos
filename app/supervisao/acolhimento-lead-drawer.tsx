...
export function AcolhimentoLeadDrawer({
  ...
  function handleConfirmAppointment() {
    startTransition(async () => {
      const res = await confirmIntakeLeadAppointment(lead.id);
      setFeedback(res.success ? { type: "success", text: "Agendamento confirmado — mensagem final enviada por WhatsApp." } : { type: "error", text: res.error });
    });
  }

  function handleRejectAppointment() {
    if (!confirm("Recusar este horário e oferecer novas opções à família?")) return;
    startTransition(async () => {
      const res = await rejectIntakeLeadAppointment(lead.id);
      setFeedback(res.success ? { type: "success", text: "Horário recusado — novas opções enviadas por WhatsApp." } : { type: "error", text: res.error });
    });
  }
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        {lead.status === "pending_confirmation" && (
          <div className="mb-6 rounded-md border border-gold bg-gold/10 p-3">
            <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-soft">Aguardando confirmação do supervisor</h4>
            <p className="mb-3 text-xs text-ink-soft">
              A família já escolheu o horário e o agendamento foi reservado na agenda. Confirme para avisar a família por WhatsApp, ou recuse para liberar o horário e oferecer novas opções.
            </p>
            <div className="flex gap-2">
              <button type="button" disabled={isPending} onClick={handleConfirmAppointment} className="rounded-md bg-chart px-3 py-2 text-xs font-semibold text-white hover:bg-chart-strong disabled:opacity-50">
                Confirmar agendamento
              </button>
              <button type="button" disabled={isPending} onClick={handleRejectAppointment} className="rounded-md bg-status-negative-soft px-3 py-2 text-xs font-semibold text-status-negative-text disabled:opacity-50">
                Recusar e reabrir horários
              </button>
            </div>
          </div>
        )}

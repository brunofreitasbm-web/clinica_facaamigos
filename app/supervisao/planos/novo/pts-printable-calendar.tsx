...
export function PTSPrintableCalendar({
  ...
  // Filtro de mês selecionado para a visualização ("ALL_ALLOWED" ou monthKey específico)
  const [selectedFilter, setSelectedFilter] = React.useState<string>("ALL_ALLOWED");

  const handlePrint = () => {
    window.print();
  };

  const allowedMonthsCount = monthKeys.filter((key) => monthPrintabilityMap[key]?.allowed).length;
      ...
      <div className="max-w-4xl mx-auto mb-4 space-y-3 print:hidden">
        ...
        <div className="bg-amber-950/40 border border-amber-500/30 rounded-xl p-3.5 text-xs text-amber-200/90 flex items-start gap-2.5 shadow-md">
          ...
          <div>
            ...
            <span>
              Impressão liberada apenas para o <strong>mês corrente</strong> e para o <strong>mês seguinte (com até 10 dias de antecedência do início do próximo mês)</strong>. Meses futuros permanecem bloqueados e serão liberados automaticamente no prazo.
            </span>
          </div>
        </div>
        ...
        {monthKeys.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            <button
              onClick={() => setSelectedFilter("ALL_ALLOWED")}
              type="button"
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all border ${
                selectedFilter === "ALL_ALLOWED"
                  ? "bg-indigo-600 text-white border-indigo-500 shadow"
                  : "bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700"
              }`}
            >
              Exibir Meses Liberados ({allowedMonthsCount})
            </button>
        ...
        <div className="space-y-6">
          {monthKeys.length === 0 ? (
            ...
            monthKeys.map((monthKey) => {
              ...
              // Se o filtro selecionado for um mês específico e não for este, ignora na renderização da tela
              if (selectedFilter !== "ALL_ALLOWED" && selectedFilter !== monthKey) {
                return null;
              }

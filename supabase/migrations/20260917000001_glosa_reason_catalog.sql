...
begin
  ...
  if v_insurer_id is not null then
    insert into glosa_reason_catalog (insurer_id, code, description, category, prevention_hint)
    values
      (v_insurer_id, 'PROASA-01', 'Procedimento eletivo sem autorização prévia (Anexo V, item 1)', 'autorizacao_previa',
        'Nunca realizar procedimento eletivo sem autorização ativa no Autorizador Web antes do atendimento.'),
      (v_insurer_id, 'PROASA-02', 'Cobrança em discordância com o contrato / fatura ou recurso fora do prazo (Anexo V, item 2)', 'prazo',
      ...
      (v_insurer_id, 'PROASA-03', 'Alteração ou rasura na documentação apresentada (Anexo V, item 3)', 'documentacao',
        'Conferir guia/laudo sem emendas antes de anexar ao faturamento; refazer documento rasurado.'),
      (v_insurer_id, 'PROASA-04', 'Identificação profissional incompleta nos registros (Anexo V, item 4)', 'identificacao_profissional',
        'Confirmar nome completo e registro do conselho de classe (CRP/CREFITO/CRN/CRFa) preenchidos na guia/relatório.'),
      (v_insurer_id, 'PROASA-05', 'Guia sem assinatura do beneficiário ou carimbo do profissional (Anexo V, item 5)', 'assinatura_beneficiario',
        'Não liberar checkout/faturamento sem assinatura do beneficiário (ou responsável) e carimbo do profissional na guia.'),
      (v_insurer_id, 'PROASA-06', 'Guia não devidamente preenchida (Anexo V, item 6)', 'guia_incompleta',
        'Revisar todos os campos obrigatórios da guia TISS antes do envio.'),
      (v_insurer_id, 'PROASA-07', 'Documentação exigida pelos Anexos III/III.A/III.B/III.C/IV não enviada (Anexo V, item 7)', 'anexos_obrigatorios',
      ...
      (v_insurer_id, 'PROASA-08', 'Autorização não anexada ao faturamento (Anexo V, item 8)', 'anexos_obrigatorios',

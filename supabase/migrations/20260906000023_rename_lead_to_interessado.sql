-- Renomeia o status de paciente "lead" para "interessado" — nome mais
-- amigável à realidade de clínicas de desenvolvimento infantil (a família
-- que fez contato mas ainda não tem avaliação agendada). Ver Boaspraticas.md.

alter table patients drop constraint patients_status_check;
update patients set status = 'interessado' where status = 'lead';
alter table patients alter column status set default 'interessado';
alter table patients add constraint patients_status_check
  check (status in ('interessado','avaliacao','ativo','pausado','alta','evadido','arquivado'));

-- Migra a métrica armazenada 'lead_to_eval_rate' para a nova chave. Não
-- inclui 'batch_lead_days' (faturamento) de propósito: ali "lead" é lead
-- time de exportação de lote, sem relação com o status do paciente.
update metric_snapshots set metric_key = 'interessado_to_eval_rate' where metric_key = 'lead_to_eval_rate';

-- Triggers de transição automática de status (20260906000018_reception_
-- audit_and_workflow.sql) que ainda checavam o valor antigo.
create or replace function trg_patient_lead_to_evaluation() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  -- Se o paciente estiver no status 'interessado', ao agendar a primeira sessão (avaliação ou triagem), transiciona para 'avaliacao'
  update patients
...
language plpgsql security definer set search_path = public as $$
begin
  -- Quando a sessão é concluída ('realizada' com checkout_at preenchido), se o paciente for 'interessado' ou 'avaliacao', vira 'ativo'
...
-- close_monthly_metric_snapshots (última versão em 20260906000008_pdi_50d_
-- deadline.sql) usava a coluna p.status <> 'lead' e a chave 'lead_to_eval_rate'.

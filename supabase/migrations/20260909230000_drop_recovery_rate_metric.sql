-- supabase/migrations/20260909230000_drop_recovery_rate_metric.sql
-- A clínica NÃO faz reagendamento/reposição de falta: não existe menu, fluxo
-- nem política de recuperação (ver 20260909210000_update_absence_message_template.sql
-- e lib/twilio-absence-bot.ts, que respondem à família que faltas não são
-- remarcadas). A métrica `recovery_rate` media, portanto, um processo
-- inexistente — toda "recuperação" que ela contava era só a próxima sessão
-- da grade recorrente do paciente caindo dentro da janela de 7 dias.
--
-- close_monthly_metric_snapshots é `create or replace` de corpo inteiro,
-- redefinido a cada migration que o toca (o mais recente:
-- 20260907170004_data_collection_rate_metas.sql). Este arquivo copia esse
-- corpo LITERALMENTE e remove apenas o bloco de recovery_rate e suas
-- variáveis — qualquer outra base reverteria silenciosamente
-- data_collection_rate, pts_50d_rate, review_on_time e as métricas de repasse.
...
-- Snapshots e metas já gravadas da métrica extinta: sem processo por trás,
-- ficariam como número morto no painel do gestor e em /gestor/metas.
delete from metric_snapshots where metric_key = 'recovery_rate';
delete from targets where metric_key = 'recovery_rate';

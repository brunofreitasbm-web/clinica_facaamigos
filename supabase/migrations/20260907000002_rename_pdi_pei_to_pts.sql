-- Renomeia a terminologia "PEI"/"PDI" para "PTS" (Projeto Terapêutico
-- Singular) — nome único e correto para o documento de plano de tratamento
-- do paciente. Ver Boaspraticas.md e app/supervisao/planos.

-- ── intake_steps.step_key ('pdi_construido' / 'pdi_validado' → 'pts_*') ────
alter table intake_steps drop constraint intake_steps_step_key_check;
update intake_steps set step_key = 'pts_construido' where step_key = 'pdi_construido';
update intake_steps set step_key = 'pts_validado' where step_key = 'pdi_validado';
alter table intake_steps add constraint intake_steps_step_key_check
  check (step_key in (
...
-- trg_treatment_plans_intake_sync (20260906000003_pdi_completo.sql).
create or replace function trg_treatment_plans_intake_sync() returns trigger
language plpgsql security definer set search_path = public as $f$
begin
  if tg_op = 'INSERT' then
    perform set_intake_step_complete(new.patient_id, 'pts_construido');
  end if;
  if tg_op = 'UPDATE' then
    if new.status = 'aprovado' and old.status is distinct from 'aprovado' then
      perform set_intake_step_complete(new.patient_id, 'pts_validado');
    end if;
...
-- ── pdi_review_due_at → pts_review_due_at (20260906000008_pdi_50d_deadline.sql) ──
drop trigger treatment_plans_default_review_due_at on treatment_plans;
drop function trg_treatment_plans_default_review_due_at();
drop function pdi_review_due_at(uuid);

create function pts_review_due_at(p_patient_id uuid) returns date
...
begin
  if new.review_due_at is null then
    new.review_due_at := pts_review_due_at(new.patient_id);
  end if;
  return new;
end;
$$;
...
-- ── metric_snapshots: 'pdi_50d_rate' → 'pts_50d_rate' ──────────────────────
update metric_snapshots set metric_key = 'pts_50d_rate' where metric_key = 'pdi_50d_rate';

-- close_monthly_metric_snapshots (última versão em 20260906000023_rename_
-- lead_to_interessado.sql) — troca só a chave/rótulo do PDI por PTS.

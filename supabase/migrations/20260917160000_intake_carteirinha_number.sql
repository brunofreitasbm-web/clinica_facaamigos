-- supabase/migrations/20260917160000_intake_carteirinha_number.sql
-- Adiciona número da carteirinha do plano nas solicitações de agendamento de anamnese
-- e atualiza a função `book_anamnesis_slot_atomic` para gravar a carteirinha no cadastro.

alter table anamnesis_scheduling_requests
  add column if not exists carteirinha_numero text;

-- Atualiza RPC book_anamnesis_slot_atomic para incluir carteirinha_numero no vinculo do paciente se disponivel
...
begin
  ...
  -- 5. Atualizar status da requisição
